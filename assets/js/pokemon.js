...
$(document).on('mouseover','img',function(){
    ...
    $.getJSON(urlPokemon, function(dataPokemon){
        ...
        let type = '<h5 class="card-title fw-bold">Type</h5><ul>';
        ...
        let peso = `'<h5 class="card-title fw-bold">Weight <p>${dataPokemon.weight /10} kg</p></h5>'`
        $('#pokemon-peso').html(peso);

        let estatura = `'<h5 class="card-title fw-bold">Height <p>${dataPokemon.height /10} kg</p></h5>'`
        $('#pokemon-estatura').html(estatura);

    }).fail(function(){
        Swall.fire({
            title:"Ups!!!",
            text:"Al parecer tenemos problemas con la conexion",
            icon:"warning"
        })
    });
