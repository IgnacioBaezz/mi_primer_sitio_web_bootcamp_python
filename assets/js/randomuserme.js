function randomuserme() {
    /*
    Crear la estructura de la tabla, obteniendo la referencia del lugar 
    donde se ubicara la tabla dinamica.
    */
    let tabla_usuarios = document.getElementById('tabla_randomuserme');

    //Crear la estructura basica de una tabla
    let tabla = document.createElement('table');
    let thead = document.createElement('thead');
    let tbody = document.createElement('tbody');

    //Definir los encabezados que tendra la tabla
    let encabezados = ['ID', 'NOMBRE', 'EMAIL', 'PAIS', 'EDAD', 'IMAGEN'];

    //Crear la fila que tendra los encabezados...
    ...
    let urlRandomUserMe = `https://randomuser.me/api/?results=${cantidad}&nat=gb,us`;
    ...
    .then(data => {
        ...
        for(const user of data.results){
            ...
            src_imagen.setAttribute('class','img-roundered')
    ...
    tabla.setAttribute('class','table table-hover table-bordered');
    thead.setAttribute('class','table-dark text-center fw-bold');
    tbody.setAttribute('class','text-center fw-medium');
}